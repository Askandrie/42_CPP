 ...
 * @ Modified by: Aska
 * @ Modified time: 2025-05-04 16:47:40
 * @ Description:
 */
...
Bureaucrat::Bureaucrat(std::string name, int grade) : _name(name), _grade(grade)
{
	...
	std::cout << IR3G5B3 "Bureaucrat constructor "
	          << "" << this->_name << " with grade " << this->_grade << RESET << std::endl;
}
...
bool Bureaucrat::signForm(AForm &form) const
{
	if (this->_grade <= 150)
	{
		std::cout << BYEL "Bureaucrat " << this->_name << " signed the form " << form.getName() << RESET << std::endl;
		return true;
	}
	else
	{
		std::cout << BYEL "Bureaucrat " << this->_name << " couldn't sign the form " << form.getName() << RESET << std::endl;
		return false;
	}
}
