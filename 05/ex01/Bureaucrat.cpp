 ...
 * @ Modified by: Aska
 * @ Modified time: 2025-04-22 22:50:52
 * @ Description:
 */
...
bool Bureaucrat::signForm(Form &form) const
{
	if (this->_grade <= 150)
	{
		std::cout << BYEL "Bureaucrat " << this->_name << " signed the form "
		          << form.getName() << RESET << std::endl;
		return true;
	}
	else
	{
		std::cout << BYEL "Bureaucrat " << this->_name
		          << " couldn't sign the form " << form.getName() << RESET
		          << std::endl;
		return false;
	}
}
