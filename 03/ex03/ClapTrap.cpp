...
ClapTrap::ClapTrap()
{
	...
	std::cout << "ClapTrap\t" << HMAG << name << RESET << "\tconstructed"
	          << std::endl;
}
...
{
	*this = origin;
	std::cout << "ClapTrap\t" << HMAG << name << RESET << "\tcopied"
	          << std::endl;
}

ClapTrap::~ClapTrap()
{
	std::cout << "ClapTrap\t" << HMAG << name << RESET << "\tdestroyed"
	          << std::endl;
}
...
void ClapTrap::takeDamage(unsigned int amount)
{
	...
	std::cout << "ClapTrap " << HMAG << name << RESET << " took " << RED
...
void ClapTrap::attack(const std::string &target)
{
	if (hitPoints == 0 || energyPoints == 0)
	{
		if (hitPoints == 0)
			std::cout << "ClapTrap " << HMAG << name << RESET
			          << " can't attack because it's out of hit points"
			          << std::endl;
		if (energyPoints == 0)
			std::cout << "ClapTrap " << HMAG << name << RESET
			          << " can't attack because it's out of energy points"
			          << std::endl;
		return;
	}
	std::cout << "ClapTrap " << HMAG << name << RESET << " attacks " << MAG
	          << target << RESET << ", causing " << YEL << attackDamage << RESET
...
void ClapTrap::beRepaired(unsigned int amount)
{
	if (hitPoints == 0 || energyPoints == 0)
	{
		if (hitPoints == 0)
			std::cout << "ClapTrap " << HMAG << name << RESET
			          << " can't heal because it's out of hit points"
			          << std::endl;
		if (energyPoints == 0)
			std::cout << "ClapTrap " << HMAG << name << RESET
			          << " can't heal because it's out of energy points"
			          << std::endl;
		return;
	}
	hitPoints += amount;
	if (hitPoints > DEFAULT_CLAPTRAP_HIT_POINTS)
		hitPoints = DEFAULT_CLAPTRAP_HIT_POINTS;
	std::cout << "ClapTrap " << HMAG << name << RESET
	          << " repairs itself, regaining " << HGRE << amount << RESET
	          << " hit points!" << std::endl;
}
