...
{
	std::cout << BYEL << "Ice\t" << RESET << "\tconstructor:\t" << (long)this
	          << std::endl;
}

Ice::Ice(Ice const &ori) : AMateria(ori.getType())
{
	std::cout << BYEL << "Ice\t" << RESET << "\tcopy const.:\t" << (long)this
	          << std::endl;
}

Ice::~Ice()
{
	std::cout << BYEL << "Ice\t" << RED << "\tdestructor:\t" << RESET
	          << (long)this << std::endl;
}
...
{
	std::cout << BR0G2B5 << "* shoots an ice bolt at " << target.getName()
	          << " *" << RESET << std::endl;
}