...
Contact::Contact()
    : Index(0), FirstName(""), LastName(""), Nickname(""), PhoneNumber(""),
      DarkestSecret("")
{
}

Contact::Contact(int index, const std::string &first_name,
                 const std::string &last_name, const std::string &nickname,
                 const std::string &phone_number,
                 const std::string &darkest_secret)
    : Index(index), FirstName(first_name), LastName(last_name),
      Nickname(nickname), PhoneNumber(phone_number),
...
void Contact::PrintContact()
{
	...
	std::cout << HBLU << "Secret\t\t: " << WHT << DarkestSecret << RESET
	          << std::endl;
}
